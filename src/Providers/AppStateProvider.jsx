...
const AppStateProvider = ({children}) =>{
  ...
  const [orders, setOrders] = useState([]); // 상태를 가지고 있도록 한다.
  ...
  const addToOrder = useCallback((id) => {
    setOrders(orders =>{
      //id 가 이미 있다면 qunatity만 올려준다.
      const finded = orders.find(orders => orders.id === id); // id를 선택하게 되면 orders의 목록해 해당 id가 있느지 검색한다.

      if (finded===undefined){ //만약에 없다면 
        return [...orders, {id, quantity: 1}]; // orders {} 안에 있는 내용을 추가하여 return한다.
      } else{ //만약에 있는 경우에는
        return orders.map(order =>{ //map을 돌면서 order를 꺼낸 뒤
          if (order.id===id){ //같은 아이디라면 quantity를 1증가시켜주고
            return {
              id,
              quantity : order.quantity + 1,
            };
          }else{ //아닌것들은 그대로 return한다.
            return order;
          }
        }) 
      }
  ...
  const remove = useCallback((id) => {
    setOrders(orders=>{
      return orders.filter(order => order.id !== id); // 인자로 들어온 id가 다른 것만 다시 리턴해준다. 즉, remove 해당 id와 같은 맥락이다.
    })
  },[]);
  const removeAll = useCallback(()=>{
    setOrders([]); //다시 초기화 해버린다.
  },[]);
